...
void Stack_Init( Stack *stack ) {
	...
	//Pokud ma stack hodnotu NULL, tak vytiskne errorove hlaseni SERR_INIT
	if(stack == NULL)
	{
	...
	{
		//Nastavim hodnotu -1, ktera odpovida prazdnemu stacku
		stack->topIndex = -1;  
	}
}
...
int Stack_IsEmpty( const Stack *stack ) {
	...
	//Pokud je hodnota stacku -1, zasobnik je prazdny
...
int Stack_IsFull( const Stack *stack ) {
	...
	//Pokud ma stack hodnotu STACK_SIZE-1 je plny (Musime zapocitat ukoncovaci znak)
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
	...
	//Osetreni neprazdnosti zasobniku
	if(stack == NULL)
	{
		//Pokud je stack prazdny, tak vytiskne errorove hlaseni SERR_TOP
		Stack_Error(SERR_TOP);
		return;
	}
	else
	{
		//Vratime prvek stack array, kteremu nalezi hodnota topIndex pres ukazatel dataPtr
...
void Stack_Pop( Stack *stack ) {
	...
	//overeni ze stack neni prazdny
    if(!Stack_IsEmpty(stack))
	{
		//Snizime topIndex a tim odstranime predesly stack top prvek
		stack->topIndex--;
	}
...
void Stack_Push( Stack *stack, char data ) {
	...
	//Overeni zda je stack plny
    if(Stack_IsFull(stack))
	{
		//Pokud je plny, na vystup se vypise errorove hlaseni (SERR_PUSH)
		Stack_Error(SERR_PUSH);
		return;
	}
	else
	{
		//Pokud plny neni, posuneme topIndex dopredu a na tento index ve stacku predame hodnoty 
		stack->topIndex++;
